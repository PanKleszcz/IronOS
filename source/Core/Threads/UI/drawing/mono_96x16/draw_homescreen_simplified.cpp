...
void ui_draw_homescreen_simplified(TemperatureType_t tipTemp) {
  ...
  if (Display::getRotation()) {
    Display::drawArea(54, 0, 42, 16, buttonAF);
    Display::drawArea(12, 0, 42, 16, buttonBF);
    Display::setCursor(0, 0);
    ui_draw_power_source_icon();
  } else {
    Display::drawArea(0, 0, 42, 16, buttonA);  // Needs to be flipped so button ends up
    Display::drawArea(42, 0, 42, 16, buttonB); // on right side of screen
  ...
  if (tempOnDisplay || tipDisconnectedDisplay) {
    ...
    if (Display::getRotation()) {
      ...
      Display::fillArea(55, 0, 41, 16, 0); // clear the area for the temp
      Display::setCursor(56, 0);
    } else {
      Display::fillArea(0, 0, 41, 16, 0); // clear the area
      Display::setCursor(0, 0);
    }
    ...
    if (!tipDisconnectedDisplay) {
      ...
      if (Display::getRotation()) {
        // in right handed mode we want to draw over the first part
        Display::drawArea(54, 0, 42, 16, disconnectedTipF);
      } else {
        Display::drawArea(0, 0, 42, 16, disconnectedTip);
      }
    }
  }
}
